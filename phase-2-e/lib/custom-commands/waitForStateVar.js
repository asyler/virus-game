...
WaitForState.prototype.phaserComponentFound = function(result, now) {
  var defaultMsg = 'Found "%s" after %d milliseconds.';
...
WaitForState.prototype.phaserComponentNotFound = function(result, now) {
  ...
  var defaultMsg = 'Timed out while waiting for "%s" after %d milliseconds.';
...
WaitForState.prototype.getProtocolCommand = function (callback) {
  var self = this;
  var varName = this.selector;
  return this.protocol.execute(
    function (varName) {
      return typeof window.Phaser.GAMES[0].state.getCurrentState()[varName] !== 'undefined';
    },
    [varName],
    function (result) {
